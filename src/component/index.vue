...
<script>
  export default {
    mounted() {
      ...
      this.legend_alarm('实验教学楼2号', '4.5', '1560', '200', 500);
      this.legend_alarm('实验教学楼3号', '8.1', '100', '100', 300);
      this.legend_alarm('实验教学楼4号', '12.8', '1200', '800', 200);
    },
    methods: {
    ...
    `
      ...
      legend_people(img, value, left, top) {
        var style;
        var trian;
        if (value <= 1) {
          style = 'bg-gray-ccc';
    ...
    `
      ...
      legend_alarm(img, value, left, top, size) {
        var style_bg;
        var style_border;
        if (value <= 3) {
          style_bg = 'bg-blue';
