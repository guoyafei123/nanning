...
export default {
    ...
    indexToAlarmTroubel() {
      ...
      if (type == 1) {
        ...
        this.indexToAlarmTroubel[0].alarms.forEach(element => {
          ...
          let alarms = this.addalarm(
            name,
            "12",
            this.listenerScale,
            [element.pointX, element.pointY],
            alarmtype
          );
  ...
  methods: {
      ...
      if (unitid != null && unitid != 0) {
        ...
        this.$fetch("/api/unit/queryById", this.queryUnit)
          .then(response => {
            if (response) {
              if (
                response.data.unit.pointX != 0 &&
                response.data.unit.pointY != 0
              ) {
                sessionStorage.unitpointX = response.data.unit.pointX;
                sessionStorage.unitpointY = response.data.unit.pointY;
                this.mp.setCenter(
                  new BMap.Point(
                    response.data.unit.pointX,
                    response.data.unit.pointY
                  )
                );
              }
            }
    ...
    addalarm(img, value, size, p, type) {
      ...
      alarm.prototype.initialize = function(map) {
        ...
        div.style.position = "absolute";
        $(div).append(
          that.legend_alarm(this._img, this._value, this._size, this._type)
        );
      ...
      if (value == 66) {
        html =
          `
						<div data-toggle="tooltip" data-placement="top" title="` +
          name +
          `" class="legend-people newpeople newpeoplealarm" style="top:-15px;left:-15px">
						</div>
					`;
      }
      return html;
    ...
    path_callpolice(type) {
      if (type == 1) {
        this.$fetch("/api/alarm/getAlarmList").then(response => {
          if (response) {
            ...
            this.$fetch("/api/alarm/getAlarmList", {
              ...
              if (response) {
                ...
                getList.forEach(element => {
                  ...
                  var b = element.pointY;
                  var name = this.toname(element);
                  this.mp.addOverlay(this.addpeople("", "10", [a, b], name));
                });
              }
            });
          }
        ...
        this.$fetch("/api/alarm/queryALarmHeatMap?count=1000").then(
          response => {
            if (response) {
    ...
    path_danger(type) {
      if (type == 1) {
        this.$fetch("/api/trouble/troubleList").then(response => {
          if (response) {
            var total = response.data.pager.totalRow;
            this.$fetch("/api/trouble/troubleList", {
              ...
              if (response) {
                ...
                getList.forEach(element => {
                  ...
                  var b = element.pointY;
                  var name = this.toname(element);
                  this.mp.addOverlay(this.addpeople("", "5", [a, b], name));
                });
              }
            });
          }
        ...
        this.$fetch("/api/trouble/queryTroubleHeatMap?count=1000").then(
          response => {
            if (response) {
              let points = response.data.points;
              let heatmapOverlay = new BMapLib.HeatmapOverlay({ radius: 50 });
    ...
    path_information() {
      this.$fetch("/api/device/deviceList").then(response => {
        if (response) {
          var total = response.data.pager.totalRow;
          this.$fetch("/api/device/deviceList", {
            ...
            if (response) {
              ...
              getList.forEach(element => {
                ...
                var b = element.pointY;
                var name = this.toname(element);
                this.mp.addOverlay(this.addpeople("", "11", [a, b], name));
              });
            }
          });
        }
    ...
    toPersonnel() {
      this.$fetch("/api/lbs/query_unit_staff_location").then(response => {
        if (response) {
  ...
  mounted() {
    ...
    if (sessionStorage.unitpointX != undefined) {
      this.mp.setCenter(
        new BMap.Point(sessionStorage.unitpointX, sessionStorage.unitpointY)
      );
    }
      ...
      if (that.routepath == "/index") {
        for (var i = 0; i < that.alarmsArray.length; i++) {
          ...
          that.alarmsArray[i][0] = that.addalarm(
            ...
            "12",
            that.listenerScale,
            [that.alarmsArray[i][1].pointX, that.alarmsArray[i][1].pointY],
            alarmtype
          )[0];
