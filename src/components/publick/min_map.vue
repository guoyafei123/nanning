	    ...
	    <div class="min-cont">
		    <ul class="min-map-list list-unstyled">		    	
		    	<li class="fire-list">
		    		...
		    		<figure>
			    		<h5>
			    			<i class="icon iconfont icon-jianzhu-xian-"></i> <span>瑞和家园1号楼</span>
			    		</h5>	
	                </figure>
		    		<!-- 关闭按钮 -->
			    	...
			    	<div class="popup-map-min-point">
				        <div class="my-tooltip"><span>气体灭火器</span></div>
	...
	export default {
      ...
      mounted(){
      	$(function() {
			var page = 1;
			var i = 3;
			var conheight = $('.min-cont').height();
			var lilen = $('img').length;
			var len = Math.ceil( lilen / i );
			// 向上
			$('.arrow-next').click(function() {
				if( page == len ) {
				$('.min-map-list').animate({top:0},'slow');
				page = 1;
				}else{
				$('.min-map-list').animate({top:'-=' + conheight},'slow');
				page++;
				}
			});
			// 向下
			$('.arrow-prev').click(function() {
				if( page == 1 ) {
				$('.min-map-list').animate({top:'-=' + conheight*(len - 1)},'slow');
				page = len;
				}else{
				$('.min-map-list').animate({top:'+=' + conheight},'slow');
				page--;
